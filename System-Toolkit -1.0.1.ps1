...
function Activate-Windows {
    Write-Host "`n[+] Activating Windows via online script..." -ForegroundColor Yellow
    try {
...
function Show-Menu {
	...
	Write-Host "22. Main Menu" -ForegroundColor Yellow
    Write-Host "23. Exit" -ForegroundColor Red
...
do {
    Show-Menu
    $choice = Read-Host "`nEnter your choice (1-24)"
    ...
    switch ($choice) {
		...
		'22' { Open-TaskManager }
		'23' { Clear-Screen }
        '24' { Write-Host "`nExiting... Thank you!" -ForegroundColor Red; Exit }
        default {
            Write-Host "`nInvalid choice. Please select 1-24." -ForegroundColor Red
            Pause
        }
    }
} while ($true)
